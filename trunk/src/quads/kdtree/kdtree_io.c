...
int kdtree_write(FILE* fout, kdtree_t* kdtree) {
	...
	if (!err) err |= write_u32_native(fout, ENDIAN_DETECTOR);
	if (!err) err |= write_u32_native(fout, nodesize);
	if (!err) err |= write_u32_native(fout, kdtree->ndata);
	if (!err) err |= write_u32_native(fout, kdtree->ndim);
	if (!err) err |= write_u32_native(fout, kdtree->nnodes);
	if (err) {
...
kdtree_t* kdtree_read(FILE* fin) {
	...
	if (!err) err |= read_u32_native(fin, &endian);
	if (!err) err |= read_u32_native(fin, &ns);
	if (!err) err |= read_u32_native(fin, &N);
	if (!err) err |= read_u32_native(fin, &D);
	if (!err) err |= read_u32_native(fin, &nnodes);
	if (err) {
		fprintf(stderr, "Couldn't read header.\n");
		return NULL;
    }
