...
int compare_candidates_2(void* v1, void* v2) {
    double diff;
...
/*
  ...
  eliminated.
*/
void dualtree_max_recurse_2(kdtree_t* xtree,
							kdtree_t* ytree,
							pl* nodes, bl* leaves,
							kdtree_node_t* ynode,
    ...
    max_bounds_function_2 bounds = callbacks->bounds;
    ...
    if (!bl_size(nodes)) {
		...
		max_start_of_results_function_2 start;
		max_end_of_results_function_2 end;
		max_result_function_2 result;
		void* result_extra;
		int i;

		if (gotoleaves && !kdtree_node_is_leaf(ytree, ynode)) {
			dualtree_max_recurse_2(xtree, ytree, nodes, leaves,
		...
		if (result) {
			...
			for (i=0; i<N; i++) {
				...
				bl_insert_sorted(keepleaves, &keepcand, compare_candidates_2);
			}
		...
		for (i=0; i<NYC; i++) {
				...
				for (xc=0; xc<2; xc++) {
					...
					if (upper > childthresh) {
						...
						if (kdtree_node_is_leaf(xtree, xchild)) {
							...
							bl_insert_sorted(childnodes, &childcand, compare_candidates_2);
			...
			dualtree_max_recurse_2(xtree, ytree, childnodes, leaves, ychild, childthresh, callbacks, gotoleaves);
