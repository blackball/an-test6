...
void sip_pixelxy2radec(sip_t* sip, double px, double py, double *a, double *d)
{
	...
	//double u = deg2rad(px) - sip->wcstan.crpix[0];
	//double v = deg2rad(py) - sip->wcstan.crpix[1];
...
//	printf("u=%lf v=%lf\n",u,v);
	...
	double x = sip->wcstan.cd[0][0] * U + sip->wcstan.cd[0][1] * V;
	double y = sip->wcstan.cd[1][0] * U + sip->wcstan.cd[1][1] * V;
//	printf("x=%lf y=%lf\n",x,y);
	...
	double rx, ry, rz;
	radec2xyz(deg2rad(sip->wcstan.crval[0]), deg2rad(sip->wcstan.crval[1]), &rx, &ry, &rz);
...
void sip_radec2pixelxy(sip_t* sip, double a, double d, double *px, double *py)
{
	// Invert CD
	double cdi[2][2];
	double inv_det = 1.0/(sip->wcstan.cd[0][0]*sip->wcstan.cd[1][1] - sip->wcstan.cd[0][1]*sip->wcstan.cd[1][0]); 
	cdi[0][0] =  sip->wcstan.cd[1][1] * inv_det;
	cdi[0][1] = -sip->wcstan.cd[0][1] * inv_det;
	cdi[1][0] = -sip->wcstan.cd[1][0] * inv_det;
	cdi[1][1] =  sip->wcstan.cd[0][0] * inv_det;

	/*
	printf(":: %lf\n",  (sip->wcstan.cd[0][0]*cdi[0][0] + sip->wcstan.cd[0][1]*cdi[1][0] ));
	printf(":: %lf\n",  (sip->wcstan.cd[0][0]*cdi[0][1] + sip->wcstan.cd[0][1]*cdi[1][1] ));
	printf(":: %lf\n",  (sip->wcstan.cd[1][0]*cdi[0][0] + sip->wcstan.cd[1][1]*cdi[1][0] ));
	printf(":: %lf\n",  (sip->wcstan.cd[1][0]*cdi[0][1] + sip->wcstan.cd[1][1]*cdi[1][1] ));
	*/

	//assert( (sip->wcstan.cd[0][0]*cdi[0][0] + sip->wcstan.cd[0][1]*cdi[1][0] ) == 1.0);
	//assert( (sip->wcstan.cd[0][0]*cdi[0][1] + sip->wcstan.cd[0][1]*cdi[1][1] ) == 0.0);
	//assert( (sip->wcstan.cd[1][0]*cdi[0][0] + sip->wcstan.cd[1][1]*cdi[1][0] ) == 0.0);
	//assert( (sip->wcstan.cd[1][0]*cdi[0][1] + sip->wcstan.cd[1][1]*cdi[1][1] ) == 1.0);
