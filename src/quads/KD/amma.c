...
/* char *pc_memory_report(void); -- moved to stats.c to eliminate dependence on stats.c */
...
#ifdef AMFAST
bool More_than_two_billion_mallocs = FALSE;
#endif
...
char *am_extended_malloc(int base_size)
{
  ...
  int max_per_bucket = 100;

  static bool already_complained = FALSE;

  if ( !already_complained &&
       Num_items_in_buckets >= max_per_bucket * NUM_COUNT_BUCKETS )
  {
    printf("*** Warning: You have mallocked so much that on average you\n"
  ...
  if ( Num_previous_am_mallocs > 2000000000 )
  {
...
#ifdef AMFAST
  if ( !More_than_two_billion_mallocs &&
       Num_previous_am_mallocs % 100000000 == 0 )
  {
    //    printf("# Just FYI am_malloc has now been called %d * 10^8 times\n",
    //	   Num_previous_am_mallocs / 100000000);
    if ( Num_previous_am_mallocs >= 2000000000 )
    {
      More_than_two_billion_mallocs = TRUE;
      //printf("# Just FYI I won't be giving any more of these am_malloc\n"
      //     "# messages as we've hit the point where I can't count them\n"
      //     "# any more. The am_malloc library will still continue\n"
      //     "# functioning happily however.\n");
    }
  }
#endif
...
char *am_malloc(int size)
{
  ...
  if ( !warned )
  {
    printf("*** WARNING! You have uncommented the VERY_CAREFUL_CHECKING\n"
	   "*** memory checking flag in amma.c . That will help detect\n"
...
void basic_am_malloc_report(void)
{
  ...
  printf("# Num_previous_am_mallocs = %d\n",Num_previous_am_mallocs);
  ...
  fprintf(stdout,"# Num calls to OS-level malloc         = %d\n",Num_mallocs);
