...
char *mk_concatenate_strings(char *s1,char *s2); /* This belongs somewhere else */
...
char *mk_concatenate_strings(char *s1,char *s2)
{
  int len1 = (int) strlen(s1);
  int len2 = (int) strlen(s2);
  int size = len1 + len2 + 1;
  char *result = AM_MALLOC_ARRAY(char,size);
  int i;
...
void add_to_help(char *s)
{
  ...
  {
    char *new_string = mk_concatenate_strings(xHelp_string,s);
    free_string(xHelp_string);
