#include "starutil.h"
#include "fileutil.h"

#define OPTIONS "hf:"
const char HelpString[] =
"quadmap -f fname > fake_catalogue.objs\n";

extern char *optarg;
extern int optind, opterr, optopt;

char *quadfname = NULL;
char *catfname = NULL;

int main(int argc, char *argv[])
{
  int argidx, argchar; //  opterr = 0;
  FILE *quadfid = NULL, *catfid = NULL;
  qidx ii,iA,iB,iC,iD,numquads;
  sidx numstars;
  dimension DimStars,DimQuads;
  double index_scale,ramin,ramax,decmin,decmax;
  off_t catposmarker = 0;
  char readStatus;
  star *tmps = NULL;

  if (argc <= 2) {
	 fprintf(stderr, HelpString);
	 return (OPT_ERR);
  }

  while ((argchar = getopt (argc, argv, OPTIONS)) != -1)
	 switch (argchar) {
	 case 'f':
		quadfname = mk_quadfn(optarg);
		catfname = mk_catfn(optarg);
		break;
	 case '?':
		fprintf(stderr, "Unknown option `-%c'.\n", optopt);
	 case 'h':
		fprintf(stderr, HelpString);
		return (HELP_ERR);
	 default:
		return (OPT_ERR);
	 }

  if (optind < argc) {
	 for (argidx = optind; argidx < argc; argidx++)
		fprintf (stderr, "Non-option argument %s\n", argv[argidx]);
	 fprintf(stderr, HelpString);
	 return (OPT_ERR);
  }

  fprintf(stderr, "quadmap: mapping quads in %s to stdout\n", quadfname);

  fopenin(quadfname, quadfid);
  readStatus = read_quad_header(quadfid,&numquads, &numstars, 
										  &DimQuads, &index_scale);
  if(readStatus == READ_FAIL) {
	 fprintf(stderr,"quadmap: read error on %s\n",quadfname);
	 return (1);
  }
  fprintf(stderr, "    (%lu quads, %lu total stars, scale=%f arcmin)\n",
			 numquads, numstars, rad2arcmin(index_scale));
  free_fn(quadfname);


  fopenin(catfname, catfid);
  readStatus = read_objs_header(catfid, &numstars, &DimStars,
										  &ramin, &ramax, &decmin, &decmax);
  if(readStatus == READ_FAIL) {
	 fprintf(stderr,"quadmap: read error on %s\n",catfname);
	 return (1);
  }
  fprintf(stderr, "  (%lu stars) (limits %lf<=ra<=%lf;%lf<=dec<=%lf.)\n",
			 numstars, rad2deg(ramin), rad2deg(ramax), rad2deg(decmin), rad2deg(decmax));
  catposmarker = ftello(catfid);
  free_fn(catfname);


  write_objs_header(stdout, DIM_QUADS*numquads,DimStars, 
						  ramin,ramax,decmin,decmax);


  tmps = mk_star();
  for(ii=0;ii<numquads;ii++) {
	 //if(is_power_of_two(ii+1))
	 //	fprintf(stderr,"  done %lu\%lu quads\r",ii+1,numquads);
	 readonequad(quadfid,&iA,&iB,&iC,&iD);
	 fseekocat(iA, catposmarker, catfid);
	 freadstar(tmps, catfid); 
	 fwritestar(tmps, stdout);
	 fseekocat(iB, catposmarker, catfid);
	 freadstar(tmps, catfid); 
	 fwritestar(tmps, stdout);
	 fseekocat(iC, catposmarker, catfid);
	 freadstar(tmps, catfid); 
	 fwritestar(tmps, stdout);
	 fseekocat(iD, catposmarker, catfid);
	 freadstar(tmps, catfid); 
	 fwritestar(tmps, stdout);
	 fflush(stdout);
  }
  free_star(tmps);


  fclose(quadfid);
	fclose(catfid);
	//basic_am_malloc_report();
	return (0);
}
