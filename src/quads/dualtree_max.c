...
void dualtree_max_recurse(blocklist* nodes, blocklist* leaves,
    ...
    if (!blocklist_count(nodes)) {
		...
		if (gotoleaves && !node_is_leaf(query)) {
			dualtree_max_recurse(nodes, leaves, query->child1, pruning_threshold, callbacks, gotoleaves);
			dualtree_max_recurse(nodes, leaves, query->child2, pruning_threshold, callbacks, gotoleaves);
			return;
		}
		...
		for (i=0; i<NYC; i++) {
			...
			dualtree_max_recurse(childnodes, leaves, ychild, childthresh, callbacks, gotoleaves);
