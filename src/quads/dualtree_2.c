...
*/
void dualtree_recurse_2(kdtree_t* xtree, kdtree_t* ytree, il* nodes, il* leaves,
					  int ynodeid, dualtree_callbacks_2* callbacks) {
	...
	if (kdtree_nodeid_is_leaf(ytree, ynodeid)) {
		start_of_results_function_2 start = callbacks->start_results;
		end_of_results_function_2 end = callbacks->end_results;
		result_function_2 result = callbacks->result;
	...
	dualtree_recurse_2(xtree, ytree, childnodes, leaves,
...
void dualtree_search_2(kdtree_t* xtree, kdtree_t* ytree,
	...
	dualtree_recurse_2(xtree, ytree, nodes, leaves, 0, callbacks);
