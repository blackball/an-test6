...
int hoggTestLeastSquareFit()
{
  ...
  printf("hoggTestLeastSquareFit: one-d chi-squared diff = %e\n",diff);
  if (diff > 1.0e-15) return 1;
  diff= fabs(1.0-aa[0]);
  printf("hoggTestLeastSquareFit: one-d parameter diff = %e\n",diff);
  if (diff > 1.0e-15) return 1;
  ...
  printf("hoggTestLeastSquareFit: two-d chi-squared diff = %e\n",diff);
  if (diff > 1.0e-15) return 1;
  diff= fabs(0.0-aa[0]);
  printf("hoggTestLeastSquareFit: first two-d parameter diff = %e\n",diff);
  diff= fabs(1.0-aa[1]);
  printf("hoggTestLeastSquareFit: second two-d parameter diff = %e\n",diff);
  if (diff > 1.0e-15) return 1;
  return 0;
}
